    ...
    def ask(self, config):
        ...
        while r is None or (self.require_value and not r):
            ...
            if not r and self.default is not None:
                ...
                print(
                    "Sorry, you must enter one of: "
                    + ", ".join(['"{}"'.format(s) for s in self.accepted_values])
                )
                r = None
        if not r:
            r = None
...
class CONFIG_BOOL(CONFIG_BASE):
    ...
    def ask(self, config):
        answers = dict(yes=True, ye=True, y=True, no=False, n=False)
        r = None
...
"""
    ...
    parser = optparse.OptionParser(
        usage=usageMsg, version=versionMsg, description=description
    )
    ...
    print(
        'You can validate the XML file with "svccfg validate {}"'.format(
            output_filename
        )
    )
